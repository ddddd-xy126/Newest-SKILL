...
async function handleGetSkillContent(args) {
  ...
  // 1. 对于 official 文档，强行跳过本地缓存摘要（因为 AI 需要真实代码而非摘要）
  const isOfficial = skillPath.includes('official_api_code_example/');
  
  // 2. 检查本地缓存
  const cached = !isOfficial ? await localCacheManager.getSkill(skillPath) : null;
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === 'get_skill_content') {
        // get_skill_content 使用本地缓存
        console.error(`[代理] 使用缓存逻辑处理: ${name}`);
        result = await handleGetSkillContent(modifiedArgs);
      } else {
        // 其他工具直接转发到服务器
        result = await forwardToolCallToServer(name, modifiedArgs);
      }
