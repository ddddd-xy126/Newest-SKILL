...
本技能每个阶段（phase-1 / phaseA / phaseB）完成后，必须在最后一段输出标准化交付报告 JSON，**格式严格以** [`../factory-supervisor-skill/contracts/delivery-schema.md`](../factory-supervisor-skill/contracts/delivery-schema.md) §3 WDP 厂报告为准（`factory="wdp"`），`mandatoryCheckpoints[]` 与 MCP 返回递一对齐。错误码命名空间 `W-xxx`。
...
> 🛤️ **执行节奏（强约束）**：
> - **第 1 步**：执行下方 §1「自检动作（自动）」3 个探测之一，**任一通过即代表 MCP 在线**。
> - **第 2 步**：自检通过 → **直接跳过 §2 / §3 的问答与配置引导**，按交付契约写出 `phase=phase-1, selfCheckPassed=true` 的报告，立即进入阶段 0/A。
> - **第 2′ 步**：自检失败 → 才进入 §2 问答引导收集 Q1~Q5，再按 §3 给出针对性配置；用户配置完成并重启 IDE 后，**必须重跑 §1 自检**直至通过，才放行进入阶段 0/A。
> - **禁止反向顺序**：在 §1 未失败时跑 Q1~Q5 是无意义打扰；在 §2 未走完时直接进阶段 A 会导致后续 MCP 工具全部报错。
