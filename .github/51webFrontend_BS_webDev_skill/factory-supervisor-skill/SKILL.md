...
[B] 🏗️ BS 步骤2: 面板还原（page_1_1，组件封装+组装挂载）  ← 依赖 [A]
[C] 🌐 WDP 阶段A+B: BIM 弹窗 Mixin                ← 依赖 [A]，可与 [B] 并行
[D] 🔌 数据厂阶段一: 封装 18 个接口              ← 可与 [A][B] 并行
[E] 🔌 数据厂阶段二: 接入 page_1_1                ← 依赖 [B][D] + key 巡检
[F] 🎩 总监终检                                    ← 依赖 [B][C][E]
─────────────────────────────────────
...
#### 4.3 调用后（厂回汇报）
- 厂的最终一段 JSON **必须严格遵循** [`contracts/delivery-schema.md`](contracts/delivery-schema.md)（顶层字段 `factory / taskId / pageKey / phase / status / selfCheckPassed / artifacts / blockers` 等）。任何偏离都视为该厂未交活，必须重发。
- 解析厂的产出（artifacts、status、blockers）
...
| 触发节点 | 巡检文件 | 失败后果 |
|---|---|---|
| BS step2（面板还原）完成 | [checks/bs-to-data-key-check.md](checks/bs-to-data-key-check.md) | 新建 blocker，禁止数据厂阶段二启动 |
...
| 厂 | 我向它输入 | 它向我返回 | 我后置做啥 |
|---|---|---|---|
| 🏗️ BS | playbook + page + step + UI 设计图 + 已知 history | 本次产出文件清单、当前轮次/面板进度 | 更新 `factories.bs`、跑 key 巡检（step2 面板还原完成后） |
...
4. 数据接入厂阶段二必须串行在对应 page 的 BS step2（面板还原）之后
...
├── GLOSSARY.md             ← 三厂统一术语表（所有歧义以此为准）
├── playbooks/              ← 5 个标准剧本
├── checks/                 ← 3 个跨厂巡检
├── contracts/              ← 三厂统一交付契约（delivery-schema.md）
├── state/                  ← 状态机模板与规范
└── prompts/                ← 调用各厂的标准措辞
```
